...
export default class Command extends BaseCommand {
  ...
  run = async (M: ISimplifiedMessage): Promise<void> => {
    ...
    if (!M.groupMetadata?.admins?.includes(this.client.user.jid))
      return void M.reply(
        "How can I remove all of the members without being an admin?"
      );
    ...
    M.groupMetadata.participants.map(async (user) => {
      if (!user.isAdmin)
        await this.client
          .groupRemove(M.from, [user.jid])
          .catch(() => console.log("Failed to remove users"));
    });
    ...
    M.groupMetadata.admins.map(async (user) => {
      if (user !== M.sender.jid && user !== this.client.user.jid)
        await this.client
          ...
          .catch(() => console.log("error removing admin"));
    });
